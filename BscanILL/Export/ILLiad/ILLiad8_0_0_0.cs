...
namespace BscanILL.Export.ILLiad
{
	...
	/// </summary>
	public class ILLiad8_0_0_0 : ILLiadBasics, IILLiad
	{
		IDatabaseSQL db = null;


		#region constructor
		internal ILLiad8_0_0_0()
			: base()
		{
		...
		{
			if (exportUnit.Article.TransactionId == null && SqlEnabled)
			{
				BscanILL.Export.ILL.TransactionPair pair = GetSqlTransactionFromIllNumber(exportUnit.Article.IllNumber);

				if (pair != null)
					exportUnit.Article.TransactionId = pair.TransactionRow.TransactionNumber;
			}

			if (exportUnit.Article.TransactionId == null)
			{
				BscanILL.UI.Dialogs.TransactionNumberDlg dlg = new BscanILL.UI.Dialogs.TransactionNumberDlg();
				exportUnit.Article.TransactionId = dlg.Open(exportUnit.Article);
			}

			if (exportUnit.Article.TransactionId != null)
			{
				FileInfo dest = new FileInfo(_settings.Export.Odyssey.ExportDir + @"\" + exportUnit.Article.TransactionId.Value + ".sent"); ;
				...
				using (FileStream stream = new FileStream(dest.FullName, FileMode.Create))
				{
					using (StreamWriter streamWriter = new StreamWriter(stream))
					{
						...
						streamWriter.WriteLine("Transaction Number: " + exportUnit.Article.TransactionId.Value);						
