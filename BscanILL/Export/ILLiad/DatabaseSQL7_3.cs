...
{
	public class DatabaseSQL7_3 : DatabaseSQLBase, IDatabaseSQL
	{
		#region variables

		private SqlDataAdapter daTransactions;
		private SqlDataAdapter daLenderAddressesALL;
		private SqlDataAdapter daLenderAddresses7_3v2;

		
		#endregion

		#region constructor
		public DatabaseSQL7_3(string sqlServerUri, string databaseName, bool windowsCredentials, string username, string password)
			: base(sqlServerUri, databaseName, windowsCredentials, username, password)
		{
			InitializeComponent();
		}

		public DatabaseSQL7_3()
			: base()
		{
			InitializeComponent();
		}
		...
		private void InitializeComponent()
		{
			...
			this.daLenderAddresses7_3v2 = new System.Data.SqlClient.SqlDataAdapter();
			// 
			// daTransactions
			// 
			...
			// 
			// LenderAddresses7_3v2
			// 
			this.daLenderAddresses7_3v2.TableMappings.AddRange(new System.Data.Common.DataTableMapping[] {
            new System.Data.Common.DataTableMapping("Table", "LenderAddressesALL", new System.Data.Common.DataColumnMapping[] {
                        new System.Data.Common.DataColumnMapping("LenderString", "LenderString"),
                        new System.Data.Common.DataColumnMapping("AddressNumber", "AddressNumber"),
                        new System.Data.Common.DataColumnMapping("NVTGC", "NVTGC"),
		...
		public override void Login()
		{
			if (sqlConnection.State != ConnectionState.Open && sqlConnection.State != ConnectionState.Connecting)
			{
				...
				/*this.cmdTransactionsSelectText = "SELECT     TransactionNumber, Username, RequestType, LoanAuthor, LoanTitle, LoanPublisher, LoanPlace, LoanDate, LoanEdition, PhotoJournalTitle, " + 
                      ...
                      "BorrowerNVTGC, TISOPaymentMethod, CCCOrder, ISOStatus, ShippingDetail, OdysseyErrorStatus, WorldCatLCNumber, Locations, FlagType,  " + 
                      "FlagNote " + 
					  "FROM         Transactions ";*/
				this.cmdTransactionsSelectText = "SELECT     TransactionNumber, Username, RequestType, LoanAuthor, LoanTitle, LoanPublisher, LoanPlace, LoanDate, LoanEdition, PhotoJournalTitle, " +
					  "PhotoJournalVolume, PhotoJournalIssue, PhotoJournalMonth, PhotoJournalYear, PhotoJournalInclusivePages, PhotoArticleAuthor, PhotoArticleTitle,  " +
					  ...
					  "PriorityShipping, Rush, CopyrightAlreadyPaid, WantedBy, SystemID, ReplacementPages, IFMCost, CopyrightPaymentMethod, ShippingOptions,  " +
					  "CCCNumber, IntlShippingOptions, ShippingAcctNo, ReferenceNumber, CopyrightComp, TAddress, TAddress2, TCity, TState, TZip, TCountry, TFax,  " +
					  "HandleWithCare, CopyWithCare, RestrictedUse, ReceivedVia, CancellationCode, BillingCategory, CCSelected, OriginalTN,  " +
					  "OriginalNVTGC, InProcessDate, InvoiceNumber, BorrowerTN, WebRequestForm, TName, TAddress3, IFMPaid, BillingAmount, ConnectorErrorStatus,  " +
					  "BorrowerNVTGC, TISOPaymentMethod, CCCOrder, ISOStatus, ShippingDetail, OdysseyErrorStatus, WorldCatLCNumber, Locations, FlagType,  " +
					  "FlagNote " +
					  "FROM         Transactions ";
				...
				if (this.singleStationSystem)
				{
					this.cmdLenderAddressesSelectText = "SELECT     LenderString, AddressNumber, NVTGC, LibraryName, Address1, Address2, Address3, Address4, BAddress1, BAddress2, BAddress3, BAddress4, Fax,  " +
						...
						"ISOPaymentMethod, DefaultShippingMethod, ISOTransport, ISOEMailAddress, ISOAutoUpdateAddress, ESPShipTo, ESPBillTo, RSSID,  " +
						"OverrideOdyssey, TrustedSenderOverride " +
		...
		private DsIlliad.LenderAddressesRow GetLenderAddress(DsIlliad ds, DsIlliad.TransactionsRow transactionsRow)
		{
			...
			if (transactionsRow.IsLendingLibraryNull() == false && transactionsRow.IsLenderAddressNumberNull() == false)
			{
				lendersRow = ds.GetLenderAdresses(transactionsRow);				
				
				if (lendersRow == null)
				{
					SqlCommand selectCmd = new SqlCommand(cmdLenderAddressesSelectText + " WHERE LenderString = '" + transactionsRow.LendingLibrary +
						"' AND AddressNumber = " + transactionsRow.LenderAddressNumber, sqlConnection);
					selectCmd.CommandTimeout = 20000;
		...
		#region CopyLenderAddressesV2ToLenderAddressesV1()
		/*private DsIlliad.LenderAddressesRow CopyLenderAddressesV2ToLenderAddressesV1(DsIlliad ds)
		{
			DsIlliad.LenderAddresses7_3v2Row r = (DsIlliad.LenderAddresses7_3v2Row)ds.LenderAddresses7_3v2.Rows[0];

			DsIlliad.LenderAddressesRow row = ds.LenderAddresses.AddLenderAddressesRow(
				r.LenderString, 
				r.AddressNumber, 
				"Lending", 
				r.IsLibraryNameNull() ? null : r.LibraryName,
				r.IsAddress1Null() ? null : r.Address1,
				r.IsAddress2Null() ? null : r.Address2,
				r.IsAddress3Null() ? null : r.Address3,
				r.IsAddress4Null() ? null : r.Address4,
				r.IsBAddress1Null() ? null : r.BAddress1,
				r.IsBAddress2Null() ? null : r.BAddress2,
				r.IsBAddress3Null() ? null : r.BAddress3,
				r.IsBAddress4Null() ? null : r.BAddress4,
				r.IsFaxNull() ? null : r.Fax,
				r.IsArielAddressNull() ? null : r.ArielAddress,
				r.IsPhoneNull() ? null : r.Phone,
				r.IsPriorityShippingNull() ? null : r.PriorityShipping,
				r.IsCopyrightPayerNull() ? null : r.CopyrightPayer,
				r.IsBillingExemptNull() ? null : r.BillingExempt,
				r.IsBillingCategoryNull() ? null : r.BillingCategory,
				r.IsLibCopyChargeNull() ? null : r.LibCopyCharge,
				r.IsLibLoanChargeNull() ? null : r.LibLoanCharge,
				r.IsLibBillingMethodNull() ? null : r.LibBillingMethod,
				r.IsNotesNull() ? null : r.Notes,
				r.IsEFTSNull() ? null : r.EFTS,
				r.IsEMailAddressNull() ? null : r.EMailAddress,
				r.IsOdysseyIPNull() ? null : r.OdysseyIP,
				r.IsOverrideIFMNull() ? null : r.OverrideIFM,
				r.IsISONull() ? null : r.ISO,
				r.IsISOIPAddressNull() ? null : r.ISOIPAddress,
				r.IsISORequesterIdNull() ? null : r.ISORequesterId,
				r.IsISOUserNameNull() ? null : r.ISOUserName,
				r.IsISOPasswordNull() ? null : r.ISOPassword,
				r.IsWebPasswordNull() ? null : r.WebPassword,
				r.IsBlockedNull() ? null : r.Blocked,
				r.IsTrustedSenderNull() ? null : r.TrustedSender,
				r.IsISOPaymentMethodNull() ? null : r.ISOPaymentMethod,
				r.IsDefaultShippingMethodNull() ? null : r.DefaultShippingMethod,
				r.IsISOTransportNull() ? null : r.ISOTransport,
				r.IsISOEMailAddressNull() ? null : r.ISOEMailAddress,
				r.IsISOAutoUpdateAddressNull() ? null : r.ISOAutoUpdateAddress,
				r.IsESPShipToNull() ? null : r.ESPShipTo,
				r.IsESPBillToNull() ? null : r.ESPBillTo,
				r.IsRSSIDNull() ? null : r.RSSID,
				r.IsOverrideOdysseyNull() ? null : r.OverrideOdyssey,
				r.IsTrustedSenderOverrideNull() ? null : r.TrustedSenderOverride);
