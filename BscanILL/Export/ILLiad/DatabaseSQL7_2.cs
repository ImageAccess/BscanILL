...
{
	public class DatabaseSQL7_2 : DatabaseSQLBase, IDatabaseSQL
	{

		#region constructor
		public DatabaseSQL7_2(string sqlServerUri, string databaseName, bool windowsCredentials, string username, string password)
			: base(sqlServerUri, databaseName, windowsCredentials, username, password)
		{			
		}

		public DatabaseSQL7_2()
			: base()
		{
		}
		#endregion
		...
		/*private void InitializeComponent()
		{
			...
			// daTransactions
			// 
			this.daTransactions.SelectCommand = this.cmdTransactionsSelect;
            ...
            new System.Data.Common.DataTableMapping("Table", "Transactions", new System.Data.Common.DataColumnMapping[] {
                        ...
                        new System.Data.Common.DataColumnMapping("SpedIns", "SpedIns"),
                        ...
                        new System.Data.Common.DataColumnMapping("RestrictedVia", "RestrictedVia"),
                        ...
                        new System.Data.Common.DataColumnMapping("BillingAmount", "BillingAmount"),
                        new System.Data.Common.DataColumnMapping("ConnectorErrorStatus", "ConnectorErrorStatus"),
                        ...
                        new System.Data.Common.DataColumnMapping("CCCOrder", "CCCOrder"),
			...
			// 
			this.daLenderAddressesALL.SelectCommand = this.cmdLenderAddressesSelect;
            ...
            new System.Data.Common.DataTableMapping("Table", "LenderAddressesALL", new System.Data.Common.DataColumnMapping[] {
                        ...
                        new System.Data.Common.DataColumnMapping("DefaultShippingMethod", "DefaultShippingMethod"),
		...
		public override void Login()
		{
			if (sqlConnection.State != ConnectionState.Open && sqlConnection.State != ConnectionState.Connecting)
			{
				...
				this.cmdTransactionsSelectText = "SELECT     TransactionNumber, Username, RequestType, LoanAuthor, LoanTitle, LoanPublisher, LoanPlace, LoanDate, LoanEdition, PhotoJournalTitle, " +
					"PhotoJournalVolume, PhotoJournalIssue, PhotoJournalMonth, PhotoJournalYear, PhotoJournalInclusivePages, PhotoArticleAuthor, PhotoArticleTitle,  " +
					"CitedIn, CitedTitle, CitedDate, CitedVolume, CitedPages, NotWantedAfter, AcceptNonEnglish, AcceptAlternateEdition, TransactionStatus,  " +
					"TransactionDate, ISSN, ILLNumber, ESPNumber, LendingString, BaseFee, PerPage, Pages, DueDate, RenewalsAllowed, SpecIns, Pieces,  " +
					"LibraryUseOnly, AllowPhotocopies, LendingLibrary, ReasonForCancellation, CallNumber, Location, Maxcost, ProcessType, ItemNumber,  " +
					"LenderAddressNumber, Ariel, Patron, PhotoItemAuthor, PhotoItemPlace, PhotoItemPublisher, PhotoItemEdition, DocumentType, InternalAcctNo,  " +
					"PriorityShipping, Rush, CopyrightAlreadyPaid, WantedBy, SystemID, ReplacementPages, IFMCost, CopyrightPaymentMethod, ShippingOptions,  " +
					"CCCNumber, IntlShippingOptions, ShippingAcctNo, ReferenceNumber, CopyrightComp, TAddress, TAddress2, TCity, TState, TZip, TCountry, TFax,  " +
					"TEMailAddress, TNumber, HandleWithCare, CopyWithCare, RestrictedUse, ReceivedVia, CancellationCode, BillingCategory, CCSelected, OriginalTN,  " +
					"OriginalNVTGC, InProcessDate, InvoiceNumber, BorrowerTN, WebRequestForm, TName, TAddress3, IFMPaid, BillingAmount, ConnectorErrorStatus,  " +
					"BorrowerNVTGC, TISOPaymentMethod, CCCOrder, ShippingDetail, ISOStatus, OdysseyErrorStatus, WorldCatLCNumber, Locations " +
					"FROM         Transactions ";

				if (this.singleStationSystem)
				{
					this.cmdLenderAddressesSelectText = "SELECT     LenderString, AddressNumber, NVTGC, LibraryName, Address1, Address2, Address3, Address4, BAddress1, BAddress2, BAddress3, BAddress4, Fax, " +
						"ArielAddress, Phone, PriorityShipping, CopyrightPayer, BillingExempt, BillingCategory, LibCopyCharge, LibLoanCharge, LibBillingMethod, Notes, EFTS," +
						"EMailAddress, OdysseyIP, OverrideIFM, ISO, ISOIPAddress, ISORequesterId, ISOUserName, ISOPassword, WebPassword, Blocked, TrustedSender, " +
						"ISOPaymentMethod, DefaultShippingMethod, ISOAutoUpdateAddress, ISOTransport, ISOEMailAddress, ESPShipTo, ESPBillTo, RSSID " +
		...
		{
			Progress_Changed(null, -1, "Getting request from ILLiad SQL server...");
			...
			using (SqlDataReader reader = selectCmd.ExecuteReader())
			{
				if (reader.Read())
				{
					requestArray = new object[reader.FieldCount];
					int count = reader.GetValues(requestArray);
				}
			...
			{
				AdjustRequestArray(requestArray);
				DsIlliad.TransactionsRow transactionsRow = (DsIlliad.TransactionsRow) ds.Transactions.Rows.Add(requestArray);

				Progress_Changed(null, -1, "Request from ILLiad SQL server was loaded.");

				string nvtgc = GetNVTGC(transactionsRow.Username);

				transactionsRow.BorrowerNVTGC = nvtgc;
		...
		public override List<BscanILL.Export.ILL.TransactionPair> GetRequests(string[] transactionStatuses, bool loans, bool articles)
		{
			...
			using (SqlDataReader reader = selectTransactionsCmd.ExecuteReader())
			{
				while (reader.Read())
				{
					object[] array = new object[reader.FieldCount];
					int count = reader.GetValues(array);

					AdjustRequestArray(array);

					ds.Transactions.Rows.Add(array);
				}
		...
		#region AdjustRequestArray()
		private void AdjustRequestArray(object[] requestArray)
		{
			if (requestArray[1] != System.DBNull.Value && ((string)requestArray[1]).Length > 20)
				requestArray[1] = ((string)requestArray[1]).Substring(0, 20);
			if (requestArray[69] != System.DBNull.Value && ((string)requestArray[69]).Length > 3)
				requestArray[69] = ((string)requestArray[69]).Substring(((string)requestArray[69]).Length - 3, 3);

			string shippingDetail = (requestArray[100] != System.DBNull.Value) ? (string)requestArray[100] : null;
			string isoStatus = (requestArray[101] != System.DBNull.Value) ? (string)requestArray[101] : null;

			requestArray[100] = isoStatus;
			requestArray[101] = shippingDetail;
		}
		#endregion	
		...
		private DsIlliad.LenderAddressesRow GetLenderAddress(DsIlliad ds, DsIlliad.TransactionsRow transactionsRow)
		{
			...
			if (transactionsRow.IsLendingLibraryNull() == false && transactionsRow.IsLenderAddressNumberNull() == false && transactionsRow.IsBorrowerNVTGCNull() == false)
			{
				lendersRow = ds.LenderAddresses.FindByLenderStringAddressNumberNVTGC(transactionsRow.LendingLibrary, transactionsRow.LenderAddressNumber, transactionsRow.BorrowerNVTGC);

				if (lendersRow == null)
				{
					SqlCommand selectCmd = new SqlCommand(cmdLenderAddressesSelectText + " WHERE LenderString = '" + transactionsRow.LendingLibrary +
						"' AND AddressNumber = " + transactionsRow.LenderAddressNumber + " AND NVTGC = '" + transactionsRow.BorrowerNVTGC + "'", sqlConnection);
					selectCmd.CommandTimeout = 20000;

					using (SqlDataReader sqlReader = selectCmd.ExecuteReader())
					{
						if (sqlReader.Read())
						{
							object[] array = new object[sqlReader.FieldCount];
							int count = sqlReader.GetValues(array);

							AdjustLenderAddressArray(array);

							lendersRow = (DsIlliad.LenderAddressesRow)ds.LenderAddresses.Rows.Add(array);
					...
					lendersRow = ds.LenderAddresses.FindByLenderStringAddressNumberNVTGC(transactionsRow.LendingLibrary, transactionsRow.LenderAddressNumber, "Lending");

					if (lendersRow == null)
					{
						selectCmd = new SqlCommand(cmdLenderAddressesSelectText + " WHERE LenderString = '" + transactionsRow.LendingLibrary +
							"' AND AddressNumber = " + transactionsRow.LenderAddressNumber + " AND NVTGC = 'Lending'", sqlConnection);
						selectCmd.CommandTimeout = 20000;
						...
						using (SqlDataReader sqlReader = selectCmd.ExecuteReader())
						{
							if (sqlReader.Read())
							{
								...
								array[2] = transactionsRow.BorrowerNVTGC;
								AdjustLenderAddressArray(array);
		...
		#region AdjustLenderAddressArray()
		private void AdjustLenderAddressArray(object[] array)
		{
			if (array[3] != System.DBNull.Value && ((string)array[3]).Length > 40)
				array[3] = ((string)array[3]).Substring(0, 40);
			if (array[4] != System.DBNull.Value && ((string)array[4]).Length > 40)
				array[4] = ((string)array[4]).Substring(0, 40);
			if (array[5] != System.DBNull.Value && ((string)array[5]).Length > 40)
				array[5] = ((string)array[5]).Substring(0, 40);
			if (array[6] != System.DBNull.Value && ((string)array[6]).Length > 40)
				array[6] = ((string)array[6]).Substring(0, 40);
			if (array[7] != System.DBNull.Value && ((string)array[7]).Length > 40)
				array[7] = ((string)array[7]).Substring(0, 40);
			if (array[8] != System.DBNull.Value && ((string)array[8]).Length > 40)
				array[8] = ((string)array[8]).Substring(0, 40);
			if (array[9] != System.DBNull.Value && ((string)array[9]).Length > 40)
				array[9] = ((string)array[9]).Substring(0, 40);
			if (array[10] != System.DBNull.Value && ((string)array[10]).Length > 40)
				array[10] = ((string)array[10]).Substring(0, 40);
			if (array[11] != System.DBNull.Value && ((string)array[11]).Length > 40)
				array[11] = ((string)array[11]).Substring(0, 40);

			string isoAutoUpdateAddress = (array[37] != System.DBNull.Value) ? (string)array[37] : null;
			string isoTransport = (array[38] != System.DBNull.Value) ? (string)array[38] : null;
			string isoEMailAddress = (array[39] != System.DBNull.Value) ? (string)array[39] : null;

			array[37] = isoTransport;
			array[38] = isoEMailAddress;
			array[39] = isoAutoUpdateAddress;
		}
		#endregion

		#region GetExportType()
		/*private BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType GetExportType(SqlDataReader reader)
		{
			if (reader[45].ToString().ToLower() == "lending")
			{
				if (reader[64].ToString().ToLower().Contains("ariel"))
					return BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.Ariel;
				else if (reader[64].ToString().ToLower().Contains("odyssey"))
					return BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.Odyssey;
				else if (reader[64].ToString().ToLower().Contains("email") || reader[64].ToString().ToLower().Contains("e-mail"))
					return BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.Email;
				else if (reader[64].ToString().ToLower().Contains("ftp"))
					return BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.FTP;
				else
					return BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.SaveOnDisk;
			}
			else
			{
				return BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.ILLiad;
			}
		}*/
		#endregion	

		#region GetAddress()
		/*private string GetAddress(SqlDataReader reader, BscanILL.Export.ILL.PullslipReader.IPullslip.Pullslip pullslip)
		{
			switch (pullslip.ExportType)
			{
				case BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.Ariel:
					{
						string lendingLibrary = (reader[40] != null) ? reader[40].ToString() : "";
						float lenderAddressNumber = (reader[47] != null) ? (float)Convert.ToDecimal(reader[47]) : 0.0F;

						SqlCommand sqlCommand = new SqlCommand("SELECT * FROM LenderAddressesALL WHERE LenderString = '" + lendingLibrary + "' AND AddressNumber = " + lenderAddressNumber, sqlConnection);

						SqlDataReader lenderReader = sqlCommand.ExecuteReader();

						if (lenderReader.Read())
						{
							if (lenderReader[13] != null)
								return Convert.ToString(lenderReader[13]);
							else
								throw new Exception("There is no Ariel address associated with lender for TN# " + pullslip.TransactionId + "!");
						}
						else
							throw new Exception("There is no lender associated with article TN# " + pullslip.TransactionId + " in the ILLiad SQL database!");
					}
				case BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.Email:
					{
						string lendingLibrary = (reader[40] != null) ? reader[40].ToString() : "";
						float lenderAddressNumber = (reader[47] != null) ? (float)Convert.ToDecimal(reader[47]) : 0.0F;
						...
						if (lenderReader.Read())
						{
							if (lenderReader[24] != null)
								return Convert.ToString(lenderReader[24]);
							else
								throw new Exception("There is no E-mail address associated with lender for TN# " + pullslip.TransactionId + "!");
						}
						else
							throw new Exception("There is no lender associated with article TN# " + pullslip.TransactionId + " in the ILLiad SQL database!");
					}
				case BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.Odyssey:
					{
						string lendingLibrary = (reader[40] != null) ? reader[40].ToString() : "";
						float lenderAddressNumber = (reader[47] != null) ? (float)Convert.ToDecimal(reader[47]) : 0.0F;
						...
						if (lenderReader.Read())
						{
							if (lenderReader[25] != null)
								return Convert.ToString(lenderReader[25]);
							else
								throw new Exception("There is no Odyssey address associated with lender for TN# " + pullslip.TransactionId + "!");
						}
						else
							throw new Exception("There is no lender associated with article TN# " + pullslip.TransactionId + " in the ILLiad SQL database!");
					}
				case BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.FTP:
					{
						return "";
					} 
				case BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.ILLiad:
					{
						return "";
					} 
				case BscanILL.Export.ILL.PullslipReader.IPullslip.ExportType.SaveOnDisk:
					{
						return "";
					}
				default:
