...
namespace BscanILL.Export.ILLiad
{
	...
	/// </summary>
	public class ILLiad7_3_0_0 : ILLiadBasics, IILLiad
	{
		IDatabaseSQL db = null;

		#region constructor
		internal ILLiad7_3_0_0()
			: base()
		{
		...
		{
			if (_settings.Export.ILLiad.ExportToOdysseyHelper)
			{
		...
		protected override void ProcessUpdateStacksWindow(ExportUnit exportUnit)
		{
			...
			try
			{
				...
				if (exportUnit.Article.TransactionId.HasValue)
				{
					...
					//SetTextFieldText(hWndSearchTransactionNumberBox, exportUnit.Article.TransactionId.ToString())
					//int result = SendMessage(hWndSearchTransactionNumberBox, WM_SETTEXT, (int)0, exportUnit.Article.TransactionId.ToString());
