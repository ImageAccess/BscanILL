...
namespace BscanILL.Export.Printing
{
	class PrintPageElement : UserControl
	{
		...
		public PrintPageElement(System.IO.FileInfo imageFile, double inchWidth, double inchHeight)
		{
			Grid grid = new Grid();
			...
			bitmapImage.UriSource = new Uri(imageFile.FullName, UriKind.Absolute);
			bitmapImage.EndInit();

			/*double bitmapMinDistance = Math.Min(bitmapImage.PixelWidth / bitmapImage.DpiX, bitmapImage.PixelHeight / bitmapImage.DpiY);
			if (bitmapMinDistance < inchWidth && bitmapMinDistance < inchHeight)
			{
