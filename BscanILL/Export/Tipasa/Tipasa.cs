...
namespace BscanILL.Export.Tipasa
{
    class Tipasa : ExportBasics
    {
        ...
        BscanILL.SETTINGS.Settings.ExportClass.TipasaClass tipasaSettings = BscanILL.SETTINGS.Settings.Instance.Export.Tipasa;
        ...
        public void ExportArticle(ExportUnit exportUnit)
        {
            try
            {
                ...
                Progress_Comment(exportUnit, "---Exporting Article (ILL#:" + exportUnit.Article.IllNumber + ", TN:" + exportUnit.Article.TransactionId + ") via Tipasa---");

                Progress_Changed(exportUnit, 5, "Checking data...");                

                //check if required fields for Tipasa delivery are filled in..
                if ((article.IllNumber == null) || (article.IllNumber.Length == 0))
                {
                    throw new Exception("ILL number not set!");
                }
                else
                if ((tipasaSettings.InstSymbol == null) || (tipasaSettings.InstSymbol.Length == 0))
                {
                ...
                this.aeWrapper.instSymbol = tipasaSettings.InstSymbol;
                ...
                this.aeWrapper.oclcRequestId = iNum.ToString();  //Tipasa is ILL# based

                //per Ed Davidson with OCLC:
                //for Tipasa the field illiadRequestId is expected to be blank                
                ...
                catch (Exception ex)
                {
                    ...
                    throw new IllException("Tipasa post has failed! " + BscanILL.Misc.Misc.GetErrorMessage(ex));
                }
            }
            ...
            {
                Notify(this, Notifications.Type.Error, ex.Message, ex);
                throw new IllException("Tipasa can't be set! " + BscanILL.Misc.Misc.GetErrorMessage(ex));
            }
        }
        #endregion
