...
{
	class FrameScanTwain : IFrameScanScanner
	{
		#region variables
		...
		#region constructor
		public FrameScanTwain(FrameScan frameScan, Scanners.Twain.TwainScanner scanner, bool isActive)
		{
			try
			{
		...
		public void Reset()
		{
			...
			{
				notifications.Notify(this, Notifications.Type.Error, "FrameScanTwain, Reset(): " + ex.Message, ex);
			}
			finally
			{
			}
		...
		public void Scan()
		{
			...
			{				
				if (sm.IsPrimaryScannerSelected)
					scanner.Scan(++this.lastOperationId, scanSettings.BookEdge);
				else
			...
			{
				notifications.Notify(this, Notifications.Type.Error, "FrameScanTwain, DoScan(): " + ex.Message, ex);
