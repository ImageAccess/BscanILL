...
namespace BscanILL.UI.Frames.Shared
{
	abstract class FrameClickMini
	{
		...
		protected Scanners.Click.ClickMiniWrapper	_scanner = null;
		...
		#region constructor
		public FrameClickMini(IFrameBase frameBase, System.Windows.Threading.Dispatcher dispatcher, MainWindow mainWindow, Scanners.Click.ClickMiniWrapper click, bool isActive)
		{
			try
			{
		...
		public void Activate()
		{
			if (_scanner != null && _isActivated == false)
			{
				...
				_scanner.ImageScannedEvent += new Scanners.Click.BitmapCompleteHnd(Scanner_ImageScanned);
				//_scanner.ImagesScanned += new Scanners.Click.ScanBitmapsCompleteHnd(Scanner_ImagesScanned);
				_scanner.ScanErrorEvent += new Scanners.Click.ScanErrorHnd(Scanner_ScanError);
				_scanner.ScannerInternalErrorEvent += new Scanners.Click.CameraInternalErrorHnd(Scanner_InternalError);
				_scanner.ScannerShutDownEvent += new Scanners.Click.CameraShutDownHnd(Scanner_ShutDownEvent);
		...
		public void Reset()
		{
			...
			{
				notifications.Notify(this, Notifications.Type.Error, "FrameClickMini, Reset(): " + ex.Message, ex);
			}
			finally
			{
			}
		...
		protected abstract void Scanner_ImageScanned(Bitmap bitmap);
		...
		{
			throw new Exception("FrameClickMini, Scanner_ImagesScanned() must be overriden");
		}
		#endregion
        ...
        void Scanner_ShutDownEvent()
        {            
            this.Dispatcher.Invoke((Action)delegate () { Scanner_ShutDownEventTU(); });
        }
        #endregion

        #region Scanner_ShutDownEventTU()        
        void Scanner_ShutDownEventTU()
        {
			      PowercycleCameras();
		    }
        #endregion
        ...
        void Scanner_InternalError()
        {            
            this.Dispatcher.Invoke((Action)delegate () { Scanner_InternalErrorTU(); });
        }
        #endregion

        #region Scanner_InternalErrorTU()        
        void Scanner_InternalErrorTU()
        {
			    PowercycleCameras();
		    }
		    #endregion
		...
		private void PowercycleCameras()
		{
			...
			catch (Exception ex)
			{
				...
				_frameBase.Notify(this, Notifications.Type.Warning, "PowercycleCameras", "FrameClickMini, PowercycleCameras(): " + ex.Message, ex);
				_frameBase.UnLock();
			}
		}
		#endregion
