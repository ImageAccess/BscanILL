...
namespace ViewPane.IP
{
	public class PreviewCreator
	{
		...
		private PreviewCreator()
		{
			...
			thread.Name = "PreviewCreator, PreviewCreator()";
		...
		{
			public readonly ViewPane.IP.IPreviewCaller			Caller;
			public readonly int									RenderingId;
			public readonly BIP.Geometry.RatioRect				ImageRect;
			public readonly FileInfo							File;
			public readonly double								Zoom;
			public readonly ImageProcessing.Resizing.ResizeMode Quality;
			public bool											IsCanceled = false;

			public WorkUnit(ViewPane.IP.IPreviewCaller caller, int renderingId, FileInfo file, BIP.Geometry.RatioRect imageRect, double zoom, ImageProcessing.Resizing.ResizeMode quality)
			{
				this.Caller = caller;
		...
		internal void GetPreviewAsync (ViewPane.IP.IPreviewCaller caller, int renderingId, bool highPriority, FileInfo file, double zoom, ImageProcessing.Resizing.ResizeMode quality)
		{
			GetPreviewAsync(caller, renderingId, highPriority, file, new BIP.Geometry.RatioRect(0, 0, 1, 1), zoom, quality);
		}

		internal void GetPreviewAsync(ViewPane.IP.IPreviewCaller caller, int renderingId, bool highPriority, FileInfo file, BIP.Geometry.RatioRect imageRect, double zoom, ImageProcessing.Resizing.ResizeMode quality)
		{
			if (isDisposed == false)
			{
				lock (queuesLocker)
				{
					WorkUnit workUnit = new WorkUnit (caller, renderingId, file, imageRect, zoom, quality);
					...
					try
					{
						...
						while (IsRunning && (workUnit != null))
						{
							try
							{
								...
								{
									Bitmap bitmap = GetPreview (workUnit.File, workUnit.ImageRect, workUnit.Zoom, workUnit.Quality);
